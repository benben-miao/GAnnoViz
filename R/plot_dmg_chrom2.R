...
#' @description Plot \bold{\emph{differentially methylated regions/genes (DMGs)}} along chromosomes,
#'   showing positions and direction (hyper/hypo) of methylation changes.
#'
#' @return A \bold{\emph{ggplot object}} visualizing DMGs along chromosomes.
#' @param dmr_file DMR/DMG table (e.g., from \bold{\emph{MethylKit}}) with columns including \code{chr}, \code{start}, optional \code{end}, and \code{meth.diff}.
...
#' )
plot_dmg_chrom2 <- function(dmr_file,
                            orientation = "vertical",
                            chrom_alpha = 0.15,
                            chrom_color = "#008888",
                            bar_height = 0.8,
  ...
  end_col <- if ("end" %in% colnames(dmr)) dmr$end else dmr$start
  dmr$pos <- if ("end" %in% colnames(dmr)) (dmr$start + dmr$end) / 2 else dmr$start
  ...
  length_map <- stats::aggregate(end_col ~ chr, data = data.frame(chr = dmr$chr, end_col = end_col), FUN = max)
  ...
  p <- ggplot2::ggplot() +
    ggplot2::geom_rect(
      data = length_map,
      ggplot2::aes(xmin = 0, xmax = end_col, ymin = ymin, ymax = ymax),
      fill = chrom_color,
      color = NA,
      alpha = chrom_alpha
    ) +
    {
      if (identical(mark_style, "point")) {
        ...
        h <- bar_height * line_height_frac / 2
        dmr$y1 <- dmr$y - h
        dmr$y2 <- dmr$y + h
